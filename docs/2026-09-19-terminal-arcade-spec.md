...
| 挡板 | `█` | U+2588，已覆盖 |
| 球 | `*` | ASCII；**不用 `O`** —— 菜单里的 `PONG` 与结束画面的 `GAME OVER` 文本含大写 O，会与球混淆（`●`U+25CF 也不在子集内） |
...
- 映射：边框/中线 `--border`，挡板 `--text`，球 `--dir`，比分 `--text` + 标签 `--text-muted`，底部提示 `--text-muted`，结束/错误 `--error`/`--success`。
...
| # | 发现 | 影响与决策 |
|---|---|---|
| 1 | **字体子集限制**：只有 ASCII + U+2500–257F + U+2588 + U+2192 + U+276F | 球不能用 `●`（U+25CF）、提示不能用 `↑↓`（U+2191/2193）⇒ 球用 `*`（避开与 `PONG`/`GAME OVER` 撞字），提示用 `Up/Down`；加字符白名单测试守住 |
